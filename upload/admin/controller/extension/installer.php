					...
					if ($root) {
						foreach ($files as $file) {
							...
							if (is_dir($file)) {
								...
								array_walk($list,function(&$each) { 
								     $each=strrpos($each, '/')? substr($each, strrpos($each, '/')+1): $each;
								 }); 
								
								
								if (!in_array((strrpos($destination, '/')?substr($destination, strrpos($destination, '/') + 1):$destination), $list)) {
